  ...
  function renderMusic(items) {
    ...
    function updateTranslationToggle() {
      ...
      if (isLyricsView && wasTranslationHidden !== shouldHideTranslation) {
        settleQueueLyricPositions(queue.querySelector('.music-lyrics-view'), { positionOnly: true });
      } else if (isLyricsView) {
    ...
    var LYRIC_CURRENT_ENTER_DURATION = 0.64;
    var LYRIC_SIDE_ENTER_DURATION = 0.66;
    var LYRIC_CASCADE_ENTER_DURATION = 0.82;
    var LYRIC_CURRENT_EXIT_DURATION = 0.72;
    var LYRIC_CURRENT_FADE_DURATION = 0.42;
    var LYRIC_SIDE_FADE_DURATION = 0.44;
    var LYRIC_CURRENT_EXIT_FADE_DURATION = 0.54;
    var LYRIC_POSITION_ONLY_DURATION = 0.52;
    var LYRIC_CASCADE_DELAY_STEP = 0.055;
    ...
    function renderQueueLyrics(panel, result, elapsed, duration) {
      ...
      panel.innerHTML = [
        ...
        windowLines.map(function (line, index) {
          ...
          return '<p class="' + className + '" data-source-index="' + line.sourceIndex + '" data-line-offset="' + distance + '" data-prev-phase="' + (previousPhase || initialPhase) + '" data-prev-y="' + initialY + '" data-prev-scale="' + initialScale.toFixed(3) + '" data-prev-opacity="' + initialOpacity.toFixed(2) + '" data-target-scale="' + targetScale.toFixed(3) + '" data-target-opacity="' + targetOpacity.toFixed(2) + '" style="' + lineStyle + '">' + primary + (line.translation ? '<span class="queue-lyric-translation">' + esc(line.translation) + '</span>' : '') + '</p>';
        }).join(''),
        '</div>'
      ].join('');
    ...
    function settleQueueLyricPositions(panel, options) {
      ...
      window.requestAnimationFrame(function () {
        ...
        if (shouldAnimate && gsap && !prefersReducedMotion()) {
          animateQueueLyricPositions(gsap, positionedLines, options);
          return;
        }
        positionedLines.forEach(function (line) {
          ...
          line.element.style.setProperty('--line-scale', getLyricTargetScale(line.element).toFixed(3));
          line.element.style.setProperty('--line-opacity', getLyricTargetOpacity(line.element).toFixed(2));
    ...
    function animateQueueLyricPositions(gsap, positionedLines, options) {
      var positionOnly = Boolean(options && options.positionOnly);
      positionedLines.forEach(function (line) {
        var element = line.element;
        if (positionOnly) {
          animateQueueLyricPositionOnly(gsap, element, line.y);
          return;
        }
        ...
        element.style.setProperty('--line-scale', state.toScale.toFixed(3));
        element.style.setProperty('--line-opacity', state.toOpacity.toFixed(2));
        ...
        gsap.to(element, {
          y: line.y,
          scale: state.toScale,
          duration: state.isCurrent ? LYRIC_CURRENT_ENTER_DURATION : state.wasCurrent ? LYRIC_CURRENT_EXIT_DURATION : cascadeDelay ? LYRIC_CASCADE_ENTER_DURATION : LYRIC_SIDE_ENTER_DURATION,
          delay: cascadeDelay,
          ease: state.isCurrent || state.wasCurrent ? 'power2.out' : cascadeDelay ? 'power3.out' : 'power4.out',
          overwrite: true,
          force3D: true
        });
        gsap.to(element, {
          opacity: state.toOpacity,
          duration: state.isCurrent ? LYRIC_CURRENT_FADE_DURATION : state.wasCurrent ? LYRIC_CURRENT_EXIT_FADE_DURATION : LYRIC_SIDE_FADE_DURATION,
    ...
    function animateQueueLyricPositionOnly(gsap, element, targetY) {
      var currentY = Number(gsap.getProperty(element, 'y'));
      var currentScale = Number(gsap.getProperty(element, 'scale'));
      var currentOpacity = Number(gsap.getProperty(element, 'opacity'));
      if (!Number.isFinite(currentY)) currentY = readNumber(element.style.getPropertyValue('--line-y'), targetY);
      if (!Number.isFinite(currentScale) || currentScale <= 0) currentScale = readNumber(element.style.getPropertyValue('--line-scale'), 1);
      if (!Number.isFinite(currentOpacity)) currentOpacity = readNumber(element.style.getPropertyValue('--line-opacity'), 1);
      element.classList.add('is-gsap-animated');
      element.style.setProperty('--line-y', targetY + 'px');
      gsap.killTweensOf(element);
      gsap.set(element, {
        y: currentY,
        scale: currentScale,
      ...
      gsap.to(element, {
        ...
        scale: currentScale,
        opacity: currentOpacity,
        duration: LYRIC_POSITION_ONLY_DURATION,
        ease: 'power3.out',
        overwrite: true,
        force3D: true
      });
    ...
    function getLyricCascadeDelay(element) {
      ...
      return Math.min(offset, 5) * LYRIC_CASCADE_DELAY_STEP;
    }
    ...
    function getLyricAnimationState(element, targetY) {
      ...
      var wasCurrent = element.dataset.prevPhase === 'current' && !isCurrent;
